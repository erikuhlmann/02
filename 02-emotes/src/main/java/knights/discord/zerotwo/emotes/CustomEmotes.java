package knights.discord.zerotwo.emotes;
...
public class CustomEmotes implements IModule {
        ...
        if (!content.startsWith("!")) {
            ...
            if (emotes.size() != 0) {
                if (messageBuilder.length() > 2000) {
                    channel.sendMessage(
                            new MessageBuilder("Only my darling can handle me like that. Don't even try.").build())
                            .queue();
    ...
    public String initEmotes(MessageReceivedEvent event) {
        ...
        String content = message.getContentRaw();
        Guild guild = event.getGuild();
        emotesAsString = Pattern.compile(":([A-Za-z0-9_\\s]*):").matcher(content).results().map(str -> {
            String output = str.group();
            output = output.replace(":", "");
            output = output.replace(" ", "_");
            return output;
        }).filter(str -> guild.getEmotesByName(str, false).size() == 0).distinct().collect(Collectors.toList());
    ...
    @Override
    public void cleanup(MessageReceivedEvent event) {
        Guild guild = event.getGuild();
